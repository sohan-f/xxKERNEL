...
* Build cache: Bazel `--disk_cache=$HOME/.cache/bazel-disk`, persisted per `lts-<flavor>-<MANIFEST_HASH>` (rotate-on-success, prefix fallback across manifest bumps, stale keys pruned). No ccache: Kleaf builds with a hermetic prebuilt clang-18 toolchain inside Bazel actions, which bypasses any compiler wrapper — Kleaf has no ccache integration (ccache only fits legacy `build/build.sh CC="ccache clang"` flows).
